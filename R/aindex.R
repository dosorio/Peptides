...
aindex<-function(seq){
  ...
  seq <- lapply(seq, function(seq){table(unlist(strsplit(seq,"")))/nchar(seq)})
  ...
  aliphatic <- unlist(lapply(seq, function(seq){sum(c(seq["A"],(2.9*seq["V"]),3.9*seq[c("L","I")]),na.rm=T)*100}))
  return(aliphatic)
}
